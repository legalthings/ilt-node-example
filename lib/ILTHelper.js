...
class ILTHelper {
  ...
  async loadChains(account, iltPublicKey) {
    const path = '/api/events/event-chains';
    const method = 'get';

    const chains = await this.sendRequest(account, path, method);

    return chains;
  }

  async loadAllShipments(account, licenseId, iltPublicKey, allowStates = []) {
    const mainprocess = await this.loadMainProcess(account, licenseId, iltPublicKey, 'live', allowStates);
    return this.loadProcessesByMainProcess(account, mainprocess.id);
  }

  async loadMainProcess(account, licenseId, iltPublicKey, activeState, allowStates = []) {
    ...
    if (process && process.current && process.current.key === activeState) {
      ...
      throw new Error('Not allowed to perform this action: ' + process.current.key + ' states: [' + allowStates.join(',') + ']');
    } else {
      return new Promise((resolve) => {
        setTimeout(async () => {
          resolve(await this.loadMainProcess(account, licenseId, iltPublicKey, activeState, allowStates));
        }, RETRY_RATE)
      });
    }
  }

  async loadShipmentProcess(account, licenseId, shipmentId, iltPublicKey, activeState, allowStates = []) {
    let process;
    try {
    ...
    if (process && process.current && process.current.key === activeState) {
      ...
      return new Promise((resolve) => {
        setTimeout(async () => {
          ...
          resolve(await this.loadShipmentProcess(account, licenseId, shipmentId, iltPublicKey, activeState, allowStates));
        }, RETRY_RATE)
      });
    }
  }
