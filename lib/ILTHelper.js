...
class ILTHelper {
  ...
  async loadAllShipments(iltAccount, account, licenseId) {
    const mainprocess = await this.loadMainProcess(iltAccount, licenseId);
  ...
  async deleteEventChain(account, licenseId) {
    const chainId = account.createEventChain(licenseId).id;
  ...
  deleteMainProcess(account, licenseId) {
    return this.deleteProcess(account, licenseId, 'main');
  }

  deleteShipmentProcess(account, licenseId, shipmentId) {
    return this.deleteProcess(account, licenseId, shipmentId);
  }

  deleteProcess(account, licenseId, process) {
    // To load the chain
    const chain = account.createEventChain(licenseId);
    const processId = chain.createProjectionId(process);
